...
class UsersViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func askForNickname(){
        let alertController = UIAlertController(title: "Socket Chat", message: "Please enter a nickname", preferredStyle: UIAlertControllerStyle.Alert)
        ...
        let OKAction = UIAlertAction(title: "OK", style: UIAlertActionStyle.Default){(action)-> Void in
            ...
            if textField.text?.characters.count == 0{
                ...
                SocketIOManager.sharedInstance.connectToServerWithNickname(self.nickname, completionHandler: { (userList) in
                    dispatch_async(dispatch_get_main_queue(), { ()-> Void in
                        if userList != nil {
                            self.users = userList
